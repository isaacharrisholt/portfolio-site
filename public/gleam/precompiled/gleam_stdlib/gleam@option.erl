...
-type option(EG) :: {some, EG} | none.

-file("src/gleam/option.gleam", 59).
-spec reverse_and_prepend(list(EV), list(EV)) -> list(EV).
...
-spec all_loop(list(option(EM)), list(EM)) -> option(list(EM)).
all_loop(List, Acc) ->
    case List of
        [] ->
...
-spec all(list(option(EH))) -> option(list(EH)).
all(List) ->
    all_loop(List, []).
...
-spec to_result(option(FD), FG) -> {ok, FD} | {error, FG}.
to_result(Option, E) ->
    case Option of
        {some, A} ->
            {ok, A};
...
-spec from_result({ok, FJ} | {error, any()}) -> option(FJ).
from_result(Result) ->
    case Result of
        {ok, A} ->
            {some, A};
...
-spec lazy_unwrap(option(FQ), fun(() -> FQ)) -> FQ.
...
-spec map(option(FS), fun((FS) -> FU)) -> option(FU).
map(Option, Fun) ->
    case Option of
        {some, X} ->
            {some, Fun(X)};
...
-spec flatten(option(option(FW))) -> option(FW).
flatten(Option) ->
    case Option of
        {some, X} ->
            X;
...
-spec then(option(GA), fun((GA) -> option(GC))) -> option(GC).
then(Option, Fun) ->
    case Option of
        {some, X} ->
            Fun(X);
...
-spec 'or'(option(GF), option(GF)) -> option(GF).
'or'(First, Second) ->
    case First of
        {some, _} ->
            First;
...
-spec lazy_or(option(GJ), fun(() -> option(GJ))) -> option(GJ).
...
-spec values_loop(list(option(GR)), list(GR)) -> list(GR).
...
-spec values(list(option(GN))) -> list(GN).
values(Options) ->
    values_loop(Options, []).
