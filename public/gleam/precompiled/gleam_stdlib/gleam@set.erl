...
-opaque set(CVJ) :: {set, gleam@dict:dict(CVJ, list(nil))}.
...
-spec contains(set(CVU), CVU) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CVW), CVW) -> set(CVW).
delete(Set, Member) ->
...
-spec fold(set(CWF), CWH, fun((CWH, CWF) -> CWH)) -> CWH.
...
-spec filter(set(CWI), fun((CWI) -> boolean())) -> set(CWI).
...
-spec drop(set(CWP), list(CWP)) -> set(CWP).
drop(Set, Disallowed) ->
...
-spec take(set(CWT), list(CWT)) -> set(CWT).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 289).
-spec order(set(CXB), set(CXB)) -> {set(CXB), set(CXB)}.
order(First, Second) ->
...
-spec intersection(set(CXG), set(CXG)) -> set(CXG).
...
-spec difference(set(CXK), set(CXK)) -> set(CXK).
...
-spec is_subset(set(CXO), set(CXO)) -> boolean().
...
-spec is_disjoint(set(CXR), set(CXR)) -> boolean().
...
-spec each(set(CXY), fun((CXY) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CVR), CVR) -> set(CVR).
insert(Set, Member) ->
...
-spec from_list(list(CWC)) -> set(CWC).
from_list(Members) ->
...
-spec map(set(CWL), fun((CWL) -> CWN)) -> set(CWN).
map(Set, Fun) ->
...
-spec union(set(CWX), set(CWX)) -> set(CWX).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(CXU), set(CXU)) -> set(CXU).
