...
-spec map_first({CMD, CME}, fun((CMD) -> CMF)) -> {CMF, CME}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CMG, CMH}, fun((CMH) -> CMI)) -> {CMG, CMI}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
