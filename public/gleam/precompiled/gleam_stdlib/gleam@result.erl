...
-spec map({ok, CMU} | {error, CMV}, fun((CMU) -> CMY)) -> {ok, CMY} |
    {error, CMV}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CNB} | {error, CNC}, fun((CNC) -> CNF)) -> {ok, CNB} |
    {error, CNF}.
...
-spec flatten({ok, {ok, CNI} | {error, CNJ}} | {error, CNJ}) -> {ok, CNI} |
    {error, CNJ}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CNQ} | {error, CNR}, fun((CNQ) -> {ok, CNU} | {error, CNR})) -> {ok,
        CNU} |
    {error, CNR}.
'try'(Result, Fun) ->
    case Result of
...
-spec unwrap({ok, CNZ} | {error, any()}, CNZ) -> CNZ.
...
-spec lazy_unwrap({ok, COD} | {error, any()}, fun(() -> COD)) -> COD.
...
-spec unwrap_error({ok, any()} | {error, COI}, COI) -> COI.
...
-spec 'or'({ok, COL} | {error, COM}, {ok, COL} | {error, COM}) -> {ok, COL} |
    {error, COM}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, COT} | {error, COU}, fun(() -> {ok, COT} | {error, COU})) -> {ok,
        COT} |
    {error, COU}.
...
-spec all(list({ok, CPB} | {error, CPC})) -> {ok, list(CPB)} | {error, CPC}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 337).
-spec partition_loop(list({ok, CPQ} | {error, CPR}), list(CPQ), list(CPR)) -> {list(CPQ),
    list(CPR)}.
...
-spec partition(list({ok, CPJ} | {error, CPK})) -> {list(CPJ), list(CPK)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CPZ}, CQC) -> {ok, CQC} | {error, CPZ}.
...
-spec replace_error({ok, CQF} | {error, any()}, CQJ) -> {ok, CQF} | {error, CQJ}.
...
-spec values(list({ok, CQM} | {error, any()})) -> list(CQM).
values(Results) ->
