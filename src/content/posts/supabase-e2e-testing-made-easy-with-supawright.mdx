...
import image0 from '../../assets/images/supabase-e2e-testing-made-easy-with-supawright/image-0.jpg'

Finally, some peace of mind.

<Image src={image0} alt="TODO" />

Photo by [Lidia Nemiroff](https://unsplash.com/@lidianemiroff?utm_source=medium&utm_medium=referral) on [Unsplash](https://unsplash.com/?utm_source=medium&utm_medium=referral)
...
Creating the records you need is generally quite easy, though you can end up with a lot of boilerplate if you have to create lots of records in foreign tables (e.g. you need to create an `order` but you need a load of `product`s first).

The real challenge comes when tracking the records that the test creates along the way. As your database grows, you might introduce new tables that get automatically populated (think: `event` tables) and suddenly you’re adding more code to _every_ test do clean up these new records.
...
Supabase is a backend-as-a-service tool that competes with the likes of Firebase and AWS Amplify, but with the key benefit of being _open source_. Supabase is awesome, and I use it all the time. If you’re interested in why I use it both at work and for personal projects, check out [this article](https://blog.withpluto.com/the-pluto-stack-b28c5e008b74) about the tech stack I use at the early-stage startup I’m currently building.
...
[Supawright](https://github.com/isaacharrisholt/supawright) is a Playwright test harness for E2E testing with Supabase.
...
In the example above, we’re telling Supawright that we care about the `public` and `other` schemas in the database. Make sure you [expose these schemas](https://supabase.com/docs/guides/api/using-custom-schemas) so the Supabase client can access them.
...
create table public.session (  
    id uuid primary key default uuid\_generate\_v4(),  
    user\_id uuid not null references public."user"(id),  
    token text,  
...
test('creates a user', async ({ supawright }) => {  
  ...
  expect(session.user\_id).toBeDefined()  
})
...
test('setup', async ({ supawright }) => {  
  ...
  await supawright  
    .supabase()  
    .from('session')  
    .insert(\[{ user\_id: user.id }, { user\_id: user.id }\])  
...
The `overrides` object is a record of schema names to a record of table names to functions that return a record of column names to values. Supawright will use these functions to create records in the database. These return an array of `Fixture`s which Supawright will use to record the records it has created.
...
I really hope you find Supawright useful. I’m very open to suggestions and contributions, so please feel free to create an issue on GitHub or get in touch with me directly on [X](https://twitter.com/IsaacHarrisHolt) or [LinkedIn](https://linkedin.com/in/isaac-harris-holt).
