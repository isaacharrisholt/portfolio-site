...
import image0 from '../../assets/images/python-to-oop-or-to-fp/image-0.jpg'
...
class Animal:  
    def \_\_init\_\_(self, name: str, logger: Logger):  
        self.name = name  
        self.logger = logger  
...
To get around the fact that our `log` function doesn’t match the `Logger` type, we’re using`functools.partial` to create a partial function that does match. This allows us to replace our logger with anything we like, as long as we can use a partial function to reduce it so that it matches our `Logger` type.
...
To get around _that_, we’d need to create initialiser functions for our animals, at which point the code gets messier and messier again.
...
Okay, so I’ll admit that this example was incredibly basic, and while it served as a good starting point for this discussion, I’d now like to give you an example of how these concepts are used in production, and I’m going to use two of my favourite Python libraries: [FastAPI](https://fastapi.tiangolo.com/) and [Pydantic](https://pydantic-docs.helpmanual.io/). FastAPI is a lightweight API framework for Python, and Pydantic is a data validation and settings management library.
...
@app.get('/foo')  
async def get\_foo(name: str, age: int) -> Foo:  
    ...  \# Some logic here  
    return Foo(  
        bar=name,  
        baz=Baz(qux=age),  
...
I’ll be writing about those in the future, and by following me on [Medium](https://medium.isaacharrisholt.com/), you won’t miss out. I also tweet about Python and my current projects on [Twitter](https://twitter.com/IsaacHarrisHolt), and (more recently) post about them on [Mastodon](https://fosstodon.org/@isaacharrisholt) too.
